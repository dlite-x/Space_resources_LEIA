...
from modules.transportation_onlyBeta import *  # To import !! ONLY WHEN USED IN calculate_energy.py!!
...
MinSlipEmpty = slip_required(mRover + mRegolith, gVal, WheelWidthVal, WheelRadiusVal, SlopeVal, wheelbaseVal,
                             heightCOGVal, nVal, kcVal, kphiVal, cVal, kVal, phiVal)  # The minimum slip when the rover is empty
...
MinSlipFull = slip_required(mRover + mRegolith, gVal, WheelWidthVal, WheelRadiusVal, SlopeVal, wheelbaseVal,
                            heightCOGVal, nVal, kcVal, kphiVal, cVal, kVal, phiVal)  # The minimum slip when the rover is full
...
def get_Alpha(cohCoeff_input, intAngle_input, extAngle_input, motor_efficiency_input, mRover_input):
    ...
    MinSlipEmpty = slip_required(mRover_input + mRegolith, gVal, WheelWidthVal, WheelRadiusVal, SlopeVal, wheelbaseVal,
                             heightCOGVal, nVal, kcVal, kphiVal, cVal, kVal, phiVal)  # The minimum slip when the rover is empty
    EnergyReqTptPerDistEmpty = energy_requirements(MinSlipEmpty, mRover_input + mRegolith, velocityExcavation, WheelWidthVal, WheelRadiusVal, SlopeVal, wheelbaseVal,
                                                heightCOGVal, gVal, nVal, kcVal, kphiVal, cVal, kVal, phiVal, motor_efficiency_input)  # [J/m] Energy requirements for transport during excavation (empty rover)
    
    #full rover
    mRegolith = 90  # [kg]
    MinSlipFull = slip_required(mRover_input + mRegolith, gVal, WheelWidthVal, WheelRadiusVal, SlopeVal, wheelbaseVal,
                                heightCOGVal, nVal, kcVal, kphiVal, cVal, kVal, phiVal)  # The minimum slip when the rover is full
    EnergyReqTptPerDistFull = energy_requirements(MinSlipFull, mRover_input + mRegolith, velocityExcavation, WheelWidthVal, WheelRadiusVal, SlopeVal, wheelbaseVal,
                                                heightCOGVal, gVal, nVal, kcVal, kphiVal, cVal, kVal, phiVal, motor_efficiency_input)  # [J/m] Energy requirements for transport during excavation (full rover)
    
    #calculate energies for full/empty rover and compute a mean value
    EnergyReqTptEmpty = EnergyReqTptPerDistEmpty * digDistance  # [J]
    EnergyReqTptFull = EnergyReqTptPerDistFull * digDistance  # [J]
    EnergyReqTpt = (EnergyReqTptEmpty+EnergyReqTptFull)/2  # [J]
    EnergyReqTpt = EnergyReqTpt * 2.77778e-7  # [kWh]
    AlphaTpt_out = EnergyReqTpt/mRegolith  # [kWh/kg]   



    digOutputs = excavationMechanics(depthM, trenchDepthM, radiusM, extAngle_input, intAngle_input, cohCoeff_input, gVal, mRegolith)

    AlphaExc_out = 2.77778e-7*digOutputs[1]  # kWh/kg
